# This file was auto-generated by Fern from our API Definition.

from __future__ import annotations

import typing

import pydantic
import typing_extensions
from ..core.pydantic_utilities import UniversalBaseModel
from .git_hub_repository_selector_kind import GitHubRepositorySelectorKind
from .linear_issue_selector_kind import LinearIssueSelectorKind
from .selector_scope import SelectorScope
from .slack_channel_selector_kind import SlackChannelSelectorKind


class IntegrationTriggerSectionOutputSelector_Github(UniversalBaseModel):
    provider: typing.Literal["github"] = "github"
    kind: typing.Optional[GitHubRepositorySelectorKind] = None
    scope: typing.Optional[SelectorScope] = None
    repositories: typing.Optional[typing.List[str]] = None

    model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)


class IntegrationTriggerSectionOutputSelector_Linear(UniversalBaseModel):
    provider: typing.Literal["linear"] = "linear"
    kind: typing.Optional[LinearIssueSelectorKind] = None
    scope: typing.Optional[SelectorScope] = None
    team_ids: typing.Optional[typing.List[str]] = None
    project_ids: typing.Optional[typing.List[str]] = None
    labels: typing.Optional[typing.List[str]] = None
    states: typing.Optional[typing.List[str]] = None

    model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)


class IntegrationTriggerSectionOutputSelector_Slack(UniversalBaseModel):
    provider: typing.Literal["slack"] = "slack"
    kind: typing.Optional[SlackChannelSelectorKind] = None
    scope: typing.Optional[SelectorScope] = None
    channels: typing.Optional[typing.List[str]] = None

    model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)


IntegrationTriggerSectionOutputSelector = typing_extensions.Annotated[
    typing.Union[
        IntegrationTriggerSectionOutputSelector_Github,
        IntegrationTriggerSectionOutputSelector_Linear,
        IntegrationTriggerSectionOutputSelector_Slack,
    ],
    pydantic.Field(discriminator="provider"),
]
